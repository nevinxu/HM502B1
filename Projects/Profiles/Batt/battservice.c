...
// ADC voltage levels
#define BATT_ADC_LEVEL_40V              468
#define BATT_ADC_LEVEL_32V              374
...
static uint8 battMeasure( void )
{
  ...
  adc = HalAdcRead( HAL_ADC_CHN_AIN7, HAL_ADC_RESOLUTION_10 );	// P0_7用作ADC_BAT
  ...
  if (adc >= BATT_ADC_LEVEL_40V)
  {
    percent = 100;
  }
  else if (adc <= BATT_ADC_LEVEL_32V)
  {
    percent = 0;
  }
  else
  {
    percent = (uint8) (adc - BATT_ADC_LEVEL_32V);
  }
