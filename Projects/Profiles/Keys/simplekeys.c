...
uint8 skKeyPressed[SEND_DATA_NUM_LEN];
  ...
  switch ( param )
  {
    case SK_KEY_ATTR:
      if ( len == sizeof ( uint8 ) ) 
      {
        ...
        VOID osal_memcpy((uint8*)skKeyPressed,(uint8*)pValue,SEND_DATA_NUM_LEN);
        
        // See if Notification/Indication has been enabled
        GATTServApp_ProcessCharCfg( skConfig, (uint8*)skKeyPressed, FALSE, 
...
bStatus_t SK_GetParameter( uint8 param, void *pValue )
{
  ...
  switch ( param )
  {
    case SK_KEY_ATTR:
      ...
      VOID osal_memcpy((uint8*)pValue,(uint8*)skKeyPressed,SEND_DATA_NUM_LEN);
      break;
  ...
  if ( pAttr->type.len == ATT_BT_UUID_SIZE )
  {
    ...
    switch ( uuid )
    {
      ...
      case SK_KEYPRESSED_UUID:
        ...
        *pLen =SEND_DATA_NUM_LEN;
        osal_memcpy(pValue,pAttr->pValue,SEND_DATA_NUM_LEN);
        break;
...
/*********************************************************************
*********************************************************************/
void testsenddata(STRREV* strdata)
{
  strdata->len='H';
  strdata->strsenddata.state[0]=0x55;
  strdata->strsenddata.state[1]=0xaa;
  VOID osal_memcpy(strdata->strsenddata.data,"0123456789abcdef",sizeof("123456789abcdef"));
  // See if Notification/Indication has been enabled
    GATTServApp_ProcessCharCfg( skConfig, (uint8*)strdata->strsenddata.data, FALSE, 
                                    simplekeysAttrTbl, GATT_NUM_ATTRS( simplekeysAttrTbl ),
                                    INVALID_TASK_ID );
}
...
void UpdataRev(STRSEND *data)
{
    static unsigned char i=0;
    VOID osal_memcpy((uint8*)skKeyPressed,&i,1);
    i++;
    VOID osal_memcpy(((uint8*)skKeyPressed+1),(uint8*)data,(SEND_DATA_NUM_LEN)-1);
